"""Does the kernel write beyond the m output rows / beyond its scratch? Oversize buffers with sentinel rows."""
import json, sys
from pathlib import Path
import torch
sys.path.insert(0, str(Path(__file__).resolve().parent))
from core import Tr3LayerExperts, TILE, K_WORDS, _SharedRef
...
for tier_name in ("tail", "keep"):
    ...
    for kind in ("decode", "prefill"):
        cap = tier.max_tokens[kind]
        big = torch.full((cap + PAD, H), 7.0, dtype=tier.outbuf[kind].dtype, device=dev)
        tier.outbuf[kind] = big[:cap]          # kernel gets [:m] views of this; rows >= cap are sentinel
        # oversized scratch with sentinel tail
        spec_n = shared[kind].numel()
        bigs = torch.full((spec_n + (64 << 20),), 0x5A, dtype=torch.uint8, device=dev)
        shared[kind] = bigs[:spec_n]
        for T in ((1, 5, 8) if kind == "decode" else (5, 100, 700)):
            ...
            big[cap:].fill_(7.0); bigs[spec_n:].fill_(0x5A); big[T:cap].fill_(7.0)
            tier.run(x, w, ids, kind); torch.cuda.synchronize()
            rows_beyond_m = int((big[T:] != 7.0).any(dim=1).sum()); rows_beyond_cap = int((big[cap:] != 7.0).any(dim=1).sum())
            scratch_beyond = int((bigs[spec_n:] != 0x5A).sum())
            print(json.dumps(dict(tier=tier_name, kind=kind, cap=cap, T=T, rows_written_beyond_m=rows_beyond_m, rows_written_beyond_capacity=rows_beyond_cap, scratch_bytes_beyond_spec=scratch_beyond)), flush=True)
