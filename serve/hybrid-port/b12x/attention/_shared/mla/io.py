...
# GLM_NOPE (GLM-5.3-Flash, qk_rope_head_dim == 0). The record is the STRICT
# [0, 288) prefix of the NVFP4 record above -- 256B packed E2M1 latent + 32B
# E4M3 group-16 scales -- optionally followed by the fp32 per-token latent
# scale at [288, 292) and 12B of 16B-alignment pad (304B stride):
#
#     [  0, 256)  packed E2M1 latent (512 x 4-bit, 32 group-16 blocks)
#     [256, 288)  32 x E4M3 group scale bytes
#     [288, 292)  fp32 per-token latent scale   } latent_scale_per_token
#     [292, 304)  zero pad (16B align)          } only
#
# There is NO RoPE sub-record, so (a) the per-token record stride is 288/304 --
# NOT the 432 of the rope-bearing NVFP4 record -- and (b) the generic NVFP4
# arm's SECOND cp.async.bulk (the rope tail) must not be issued at all.
_NVFP4_NO_ROPE_GMEM_STRIDE = 288
_NVFP4_NO_ROPE_PTS_GMEM_STRIDE = 304
# 8B-aligned source of the fp32 per-token latent scale word pair. Both NVFP4
# two-level records put it at +288: the 368B fp8-rope record keeps the SECOND
# word ([292,296); [288,292) is its rope scale), the 304B NoPE record keeps the
# FIRST word ([288,292); [292,296) is pad).
...
def _nvfp4_no_rope_gmem_stride(
    kv_gmem_stride: int, per_token_latent_scale: bool
) -> int:
    """Resolve the GLM_NOPE per-token record stride at TRACE time (pure Python).

    ``kv_gmem_stride`` (0 == unspecified) lets a caller thread
    ``traits.kv_gmem_stride`` straight through and is then AUTHORITATIVE. When
    it is omitted the stride follows ``latent_scale_per_token`` exactly the way
    ``make_unified_traits`` derives it (288 without the per-token fp32 latent
    scale, 304 with it), so the device geometry can never drift from the trait
    that sized the cache.
    """
    if kv_gmem_stride:
        if int(kv_gmem_stride) not in (
            _NVFP4_NO_ROPE_GMEM_STRIDE,
            _NVFP4_NO_ROPE_PTS_GMEM_STRIDE,
        ):
            raise ValueError(
                "GLM_NOPE kv_gmem_stride must be 288 or 304 (the rope-less "
                f"NVFP4 record); got {int(kv_gmem_stride)}"
            )
        return int(kv_gmem_stride)
    return (
        _NVFP4_NO_ROPE_PTS_GMEM_STRIDE
        if per_token_latent_scale
        else _NVFP4_NO_ROPE_GMEM_STRIDE
    )
...
def io_issue_gather(
    ...
    *,
    ...
    rope_smem_stride: cutlass.Constexpr,  # 64 (D_ROPE bf16 elems); 0 == GLM_NOPE
    ...
    no_rope: cutlass.Constexpr = False,  # GLM_NOPE: qk_rope_head_dim == 0
    ...
    group ([0, io_threads)). Defaulting to 32 keeps the decode PTX byte-identical.
      ...
      4. cp.async.bulk each entry; DSV4 issues separate NoPE (448B) and RoPE
         ...
         GLM_NOPE (``rope_smem_stride == 0`` / ``no_rope``) issues ONLY the
         288B latent copy -- its record has no rope tail and its ``kv_rope``
         smem region is 0 bytes wide, so the second copy is const_expr-elided
         rather than aimed at a zero-extent region.
    ...
    # GLM_NOPE specialization key (const_expr, resolved at TRACE time). The
    # caller already threads ``traits.d_rope`` in as ``rope_smem_stride``, and
    # d_rope == 0 is unique to the rope-less record (DSV4 and GLM_NSA are both
    # 64), so it IS the key; ``no_rope`` lets a caller state the trait outright.
    # Every rope-bearing model evaluates this to False, so their const_expr
    # branches -- and their PTX -- are untouched.
    _no_rope = bool(no_rope) or int(rope_smem_stride) == 0
    ...
    # NVFP4 NoPE: 288B (304B with the per-token latent scale), NO rope tail.
    if cutlass.const_expr(scale_format == 0):
        ...
        if cutlass.const_expr(_no_rope):
            # GLM_NOPE: the whole record IS the 288B latent+scales prefix (304B
            # when the per-token fp32 latent scale + 16B-align pad follow it).
            # _ROPE / _ROPE_SRC are deliberately left UNDEFINED: the rope bulk
            # below is const_expr-elided, and leaving the names unbound means a
            # regression that resurrected it would fail LOUDLY at trace time
            # (NameError) instead of writing 64 zero-extent copies over the
            # live mbarrier array (kv_rope is a 0-byte region for NoPE).
            _IOS = Int64(
                _nvfp4_no_rope_gmem_stride(kv_gmem_stride, per_token_latent_scale)
            )
    ...
    def _issue_payload_entry(entry: Int32, idx_raw: Int32):
        ...
        if cutlass.const_expr(packed_glm):
            ...
            # GLM_NOPE has no rope sub-record: the second copy is elided whole.
            # bulk_tx_bytes (BI*288) then already matches the bytes that land.
            if cutlass.const_expr(not _no_rope):
                cp_async_bulk_g2s_mbar(
        ...
        for _ in cutlass.range_constexpr((bi + io_threads - 1) // io_threads):
            ...
            if entry < Int32(bi):
                ...
                if cutlass.const_expr(packed_glm):
                    ...
                    # See _issue_payload_entry: NoPE elides the rope copy.
                    if cutlass.const_expr(not _no_rope):
                        cp_async_bulk_g2s_mbar(
                            kv_rope_dst_addr + entry * Int32(rope_smem_stride * 2),
                            data_base_i64 + _ROPE_SRC,
    ...
    for _ in cutlass.range_constexpr((bi + io_threads - 1) // io_threads):
        ...
        if entry < Int32(bi):
            ...
            if cutlass.const_expr(scale_format == 0):
                ...
                # NVFP4 two-level record: gather the per-token fp32 latent scale
                # into the contiguous smem kv_sc buffer that smem.py allocates
                # for latent_scale_per_token (BI x 4). BOTH two-level variants
                # need it -- gating this on fp8_rope left GLM_NOPE's kv_sc
                # UNINITIALIZED and the outer dequant scale garbage.
                #
                # The word pair at [288, 296) is 8B-aligned in both records, so
                # one nc.v2.u32 covers both; only the kept word differs:
