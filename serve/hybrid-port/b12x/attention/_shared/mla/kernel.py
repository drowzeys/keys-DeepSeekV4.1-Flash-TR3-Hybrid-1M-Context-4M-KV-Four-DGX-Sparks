...
class UnifiedDecodeKernel:
    ...
    def kernel(
        ...
        if cutlass.const_expr(t.d_rope > 0):
...
def _cache_block_stride_bytes(
    ...
    *,
    ...
    if (
        int(model_type) in (int(ModelType.GLM_NSA), int(ModelType.GLM_NOPE))
...
def run_unified_decode(
    *,
    ...
    model_type, compute_mode, scale_format = infer_model_type(
        q_head_dim,
        swa_k_cache.dtype,
        # Only a byte-typed cache has a byte-meaningful trailing extent; that
        # extent is what separates GLM_NOPE from DSV4 at q_head_dim == 512.
        kv_record_bytes=(
            int(swa_k_cache.shape[-1]) if swa_k_cache.dtype.itemsize == 1 else None
        ),
    )
    ...
    if scale_format == ScaleFormat.NVFP4_E4M3 and fp8_rope_override is None:
        ...
        if int(model_type) == int(ModelType.GLM_NOPE):
            # A NoPE record has no RoPE tail at all, so fp8_rope is never
            # meaningful here; 288 vs 304 selects the per-token latent scale.
            if record_bytes not in (288, 304):
                raise ValueError(
                    "GLM_NOPE NVFP4 cache record must be 288 or 304 bytes, "
                    f"got {record_bytes}"
                )
    ...
    if latent_scale_per_token:
        ...
        if int(model_type) == int(ModelType.GLM_NOPE):
            if int(swa_k_cache.shape[-1]) != 304:
                raise ValueError(
                    "SM120 sparse MLA decode latent_scale_per_token requires the "
                    "304-byte GLM_NOPE NVFP4 record; got "
                    f"{int(swa_k_cache.shape[-1])} bytes"
                )
        elif not bool(fp8_rope_override):
            raise ValueError(
                "SM120 sparse MLA decode latent_scale_per_token requires the "
