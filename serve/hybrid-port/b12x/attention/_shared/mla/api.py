...
# GLM_NOPE contract (q_head_dim = kv_lora_rank 512 + qk_rope_head_dim 0).
_MLA_NOPE_Q_HEAD_DIM = 512
# GLM_NOPE NVFP4 record widths: 288 B latent-only, 304 B with the per-token
# fp32 latent scale. Neither can collide with GLM_NSA's 368/432.
_NOPE_RECORD_BYTES = (288, 304)
...
def _run_sparse_mla(
    *,
    ...
    if int(scale_format_for_call or -1) == 2 and fp8_rope is None:
        ...
        if record_bytes in _NOPE_RECORD_BYTES:
            # GLM_NOPE has no RoPE sub-record, so there is no fp8-rope variant
            # to select: the width alone (288 vs 304) picks the per-token scale.
            fp8_rope_for_call = False
        elif record_bytes in (368, 432):
            fp8_rope_for_call = record_bytes == 368
        else:
            raise ValueError(
                "NVFP4 sparse MLA cache record must be 368 or 432 bytes "
                f"(GLM_NSA) or 288 or 304 bytes (GLM_NOPE), got {record_bytes}"
            )
    else:
    ...
    if int(scale_format_for_call or -1) == 2:
        record_bytes = int(kv_cache.shape[-1])
        if record_bytes in _NOPE_RECORD_BYTES:
            if fp8_rope_for_call:
                raise ValueError(
                    "KV_FP8_ROPE is meaningless for the GLM_NOPE NVFP4 record "
                    f"({record_bytes} bytes): qk_rope_head_dim == 0"
                )
        else:
    ...
    if latent_scale_per_token:
        ...
        if int(kv_cache.shape[-1]) not in (368, 304):
            raise ValueError(
                "sparse MLA latent_scale_per_token requires the fp8-rope "
                "368-byte NVFP4 record or the 304-byte GLM_NOPE record; got "
                f"{int(kv_cache.shape[-1])} bytes"
            )
    ...
    if _sm120_route:
        ...
        if q_head_dim not in (_MLA_UNIFIED_GLM_Q_HEAD_DIM, _MLA_NOPE_Q_HEAD_DIM):
            # 512 is admitted for GLM_NOPE (kv_lora_rank 512 + rope 0). It is
            # also DSV4's width, but that ambiguity is resolved downstream by
            # infer_model_type() on the NVFP4 record byte width, not here.
            raise ValueError(
                f"SM120 sparse MLA decode requires the GLM_NSA contract "
                f"(q_head_dim={_MLA_UNIFIED_GLM_Q_HEAD_DIM}) or the GLM_NOPE "
                f"contract (q_head_dim={_MLA_NOPE_Q_HEAD_DIM}); "
