    ...
    def _body(
        ...
        *,
        ...
        is_glm = cutlass.const_expr(
            t.model_type in (ModelType.GLM_NSA, ModelType.GLM_NOPE)
        )
            ...
            if loop_tiles > Int32(0):
                ...
                if cutlass.const_expr(is_glm):
                    io_issue_gather_glm_mg(
                        ...
                        d_rope=t.d_rope,
                        kv_gmem_stride=t.kv_gmem_stride,
                    )
                else:
...
def run_unified_prefill_mg(
    *,
    ...
    is_glm_nsa = model_type == ModelType.GLM_NSA
    is_glm_nope = model_type == ModelType.GLM_NOPE
    is_glm = is_glm_nsa or is_glm_nope
    ...
    expected_qdim = _GLM_HEAD_DIM if is_glm_nsa else _DSV4_HEAD_DIM
    ...
    if scale_format == ScaleFormat.NVFP4_E4M3:
        ...
        if is_glm_nope:
            if fp8_rope:
                raise ValueError(
                    "GLM_NOPE NVFP4 sparse MLA MG has no RoPE sub-record"
                )
            expected_record_bytes = 304 if latent_scale_per_token else 288
            if record_bytes != expected_record_bytes:
                raise ValueError(
                    "GLM_NOPE NVFP4 sparse MLA MG cache record must be "
                    f"{expected_record_bytes} bytes, got {record_bytes}"
                )
            fp8_rope = False
        else:
            if fp8_rope is None:
                if record_bytes not in (368, 432):
                    raise ValueError(
                        "NVFP4 sparse MLA MG cache record must be 368 or 432 "
                        f"bytes, got {record_bytes}"
                    )
    ...
    if latent_scale_per_token:
        ...
        if not bool(fp8_rope) and not is_glm_nope:
            raise ValueError(
                "SM120 sparse MLA MG prefill latent_scale_per_token requires "
